# Relay v0.3 — Implementation Plan (ARCHIVED — shipped in v0.3.0)
> Theme: Relay should work with whatever the framework builder is already using. No lock-in.
>
> **Status:** ✅ Implemented in v0.3.0. Current version is v0.4.1.
...
- [x] `relay.runners.AgentRunner` (Protocol, `@runtime_checkable`)
- [x] `relay.runners.AgentOutput` (frozen dataclass)
- [x] `relay.runners.ContextSlice` (frozen dataclass)
- [x] `relay.runners.AdapterRegistry`
- [x] `relay.runners.RawSDKAdapter`
- [x] `relay.runners.LangChainAdapter`
- [x] `relay.runners.CrewAIAdapter`
- [x] `relay.runners.AutoGenAdapter`
- [x] `relay.runners.LocalModelAdapter`
- [x] `CoreRelayPipeline.execute_step_with_runner()` (async)
- [x] `CoreRelayPipeline._build_context_slice()` (private)
- [x] Optional extras: `langchain`, `crewai`, `autogen`, `local`, `all` (updated)
- [x] Integration test suite (one adapter test through full pipeline)
- [x] `mypy --strict` passes across all of `src/`
- [x] `CHANGELOG.md` entry for v0.3.0
- [x] `README.md` updated with v0.3 usage examples
