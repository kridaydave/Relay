...
- **Layer dependency order** (lower never imports upper): `types.py` → `envelope.py` → `snapshot.py` → `validator.py` → `context_broker.py` → `budget/` + `slicer/` → `pipeline_state.py` → `pipeline_*.py` → `core_pipeline.py`
