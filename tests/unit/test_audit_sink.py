...
class TestAuditSinkProtocol:
    ...
    def test_json_log_sink_satisfies_audit_sink_protocol(self) -> None:
        """JsonLogSink must satisfy the AuditSink Protocol."""
        sink = JsonLogSink()
        assert isinstance(sink, AuditSink)

    def test_json_log_sink_satisfies_closeable_protocol(self) -> None:
        """JsonLogSink must satisfy the Closeable Protocol."""
        sink = JsonLogSink()
        assert isinstance(sink, Closeable)

    def test_fixed_audit_sink_satisfies_audit_sink_protocol(self) -> None:
        """FixedAuditSink must satisfy the AuditSink Protocol."""
        sink = FixedAuditSink()
        assert isinstance(sink, AuditSink)

    def test_fixed_audit_sink_satisfies_closeable_protocol(self) -> None:
...
class TestJsonLogSink:
    ...
    def test_emit_handles_serialization_failure_gracefully(self) -> None:
...
class TestFixedAuditSink:
    ...
    def test_multiple_emit_calls_accumulate_events(self) -> None:
