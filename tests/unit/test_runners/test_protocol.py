...
class TestAgentRunnerProtocol:
    def test_fixed_agent_runner_succeeds_and_satisfies_protocol_at_runtime(self) -> None:
    ...
    def test_object_without_run_fails_and_does_not_satisfy_protocol_at_runtime(self) -> None:
        """Bare object must not satisfy AgentRunner Protocol."""
        assert not isinstance(object(), AgentRunner)

    def test_class_without_async_run_succeeds_and_satisfies_protocol_attribute_check(self) -> None:
        ...
        """
        class SyncRunner:
            def run(self, slice: object, manifest: object) -> AgentOutput:
...
class TestAgentOutput:
    ...
    def test_raises_when_both_text_and_structured_are_empty(self) -> None:
        with pytest.raises(ValueError, match="At least one of"):
            AgentOutput(text="", structured={}, tool_calls=[], token_count=1, latency_ms=0, adapter="test")


class TestContextSlice:
    def test_context_slice_is_frozen_when_modified(self) -> None:
        from .conftest import make_test_slice
        slice_ = make_test_slice()
        with pytest.raises(Exception):
            setattr(slice_, "step", 99)

    def test_context_slice_sections_reflects_manifest_reads_when_filtered(self) -> None:
    ...
    def test_context_slice_contains_required_fields_when_initialized(self) -> None:
        sections: JSONDict = {"key": "value"}
        slice_ = ContextSlice(
            pipeline_id="p1",
            step=5,
            agent_id="a1",
